...
func Factory(conf model.MaskFactoryConfiguration) (model.MaskEngine, bool, error) {
	...
	if len(conf.Masking.Mask.RandomChoice) != 0 {
		return NewMask(conf.Masking.Mask.RandomChoice, conf.Seed, model.NewSeeder(conf.Masking.Seed.Field, conf.Seed, conf.SeedFromClock)), true, nil
	}
...
func Func(seed int64, seedField string, seedFromClock bool) interface{} {
	...
	return func(choices_ []interface{}) (model.Entry, error) {
		...
		mask := NewMask(choices, seed+callnumber, model.NewSeeder(seedField, seed+callnumber, seedFromClock))
		callnumber++
