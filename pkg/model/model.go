...
func NewMaskConfiguration() MaskConfiguration {
	return MapMaskConfiguration{[]MaskKeyStruct{}}
}
...
func (mmc MapMaskConfiguration) WithContextEntry(key string, engine MaskContextEngine) MaskConfiguration {
	...
	if len(mainEntry) == 2 {
		mmc.config = append(mmc.config, MaskKeyStruct{NewMaskConfiguration().WithContextEntry(mainEntry[1], engine).AsContextEngine(), mainEntry[0]})
		...
		mmc.config = append(mmc.config, MaskKeyStruct{engine, key})
	}
...
func (mmc MapMaskConfiguration) Entries() []MaskKeyStruct {
	return mmc.config
}
