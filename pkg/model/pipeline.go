...
func InjectConfig(skipLineOnErrorValue bool, skipFieldOnErrorValue bool, skipLineLogFileValue string) {
	skipLineOnError = skipLineOnErrorValue
	skipFieldOnError = skipFieldOnErrorValue
	skipLineLogFile = skipLineLogFileValue
}
...
func BuildPipeline(pipeline Pipeline, conf Definition, caches map[string]Cache, functions tmpl.FuncMap) (Pipeline, map[string]Cache, error) {
	...
	for _, masking := range conf.Masking {
		...
		for _, sel := range allSelectors {
			...
			if cache, ok := caches[masking.Cache]; ok {
				pipeline = pipeline.Process(&LookupValueSetter{cache, NewPackedPathSelector(sel.Jsonpath)}, errCollector)
			}
			...
			for _, maskDefinition := range allMasksDefinition {
				...
				if virtualMask.Mask.FromCache != "" {
					...
					pipeline = pipeline.Process(NewFromCacheProcess(NewPackedPathSelector(virtualMask.Selector.Jsonpath), cache, virtualMask.Preserve), errCollector)
					nbArg++
				}
				...
				for _, factory := range maskFactories {
					...
					if present {
						...
						pipeline = pipeline.Process(NewMaskEngineProcess(NewPackedPathSelector(virtualMask.Selector.Jsonpath), mask, virtualMask.Preserve), errCollector)
						nbArg++
					}
				}
				...
				for _, factory := range maskContextFactories {
					...
					if present {
						...
						pipeline = pipeline.Process(NewMaskContextEngineProcess(NewPackedPathSelector(virtualMask.Selector.Jsonpath), mask), errCollector)
						nbArg++
			...
			if cache, ok := caches[masking.Cache]; ok {
				pipeline = pipeline.Process(&LookupValueDiscarder{cache}, errCollector)
			}
	...
	for _, cleaner := range cleaners {
		pipeline = pipeline.Process(cleaner, errCollector)
	}
